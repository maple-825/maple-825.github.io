...
window.onload = function starttime() {
    time(h1, '2021,03,01');     // 在一起的时间
    ptimer = setTimeout(starttime, 1000); // 添加计时器
}

function time(obj, futime) {
    let nowtime = new Date().getTime(); // 现在时间转换为时间戳
    let futruetime = new Date(futime).getTime(); // 未来时间转换为时间戳
...
//              alert(hour)
    obj.innerHTML = "亲爱滴´ε｀～❤<br>霞宝，我们已经在一起：<br>" + day + "天" + hour + "小时" + minute + "分" + second + "秒" + "了<br><span>不管面临多大的压力,不管前面的路如何崎岖.<br>不管经历过什么,我仍坚信我们一定会克服困难.<br><span style='font-size: 37px;'>你最可爱，我说时来不及思索，而思索之后，还是这样说。</span></span>"
